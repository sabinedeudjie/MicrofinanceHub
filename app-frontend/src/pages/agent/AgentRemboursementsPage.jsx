...
import {
  ...
  ChevronDown, ChevronUp, CreditCard, User, Banknote,
} from 'lucide-react';
...
const AgentRemboursementsPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const ids = (clientsRes.data ?? []).map(c => c.id).filter(Boolean);
      setClientIds(ids);
  ...
  const openModal = async (loan) => {
    ...
    setMontant(String(nextUnpaid?.dueAmount ?? loan.monthlyPayment ?? ''));
    setAmortModal(entries);
  };

  // Re-sync amortModal when cache updates
  useEffect(() => {
  ...
  const handleEnregistrer = async (e) => {
    ...
    try {
      ...
      setSuccessLoanId(modal.id);
      setModal(null);
      setAmortCache(prev => { const n = { ...prev }; delete n[modal.id]; return n; });
      await load();
      setTimeout(() => setSuccessLoanId(null), 4000);
    } catch (err) {
  ...
  const nextInstallment = (loanId) => {
    const entries = amortCache[loanId] ?? [];
    return entries.find(e => !e.paid) ?? null;
  };

  const paidCount = (loanId) => (amortCache[loanId] ?? []).filter(e => e.paid).length;
  const totalCount = (loanId) => (amortCache[loanId] ?? []).length;

  return (
    <div className="p-6 space-y-5">

      <div>
        <h1 className="text-2xl font-bold text-gray-800">Remboursements Terrain</h1>
        <p className="text-gray-500 text-sm">Enregistrez les paiements collectés auprès de vos clients</p>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-gray-700">Prêts Actifs — Enregistrer un Paiement</h3>
            <p className="text-xs text-gray-400 mt-0.5">
              {loading ? 'Chargement…' : `${loans.length} prêt(s) actif(s) dans votre agence`}
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {loans.map(loan => {
              const isSuccess  = successLoanId === loan.id;
              const expanded   = expandedLoan === loan.id;
              const entries    = amortCache[loan.id];
              const paid       = entries ? paidCount(loan.id)  : null;
              const total      = entries ? totalCount(loan.id) : null;
              const pct        = paid != null && total ? Math.round((paid / total) * 100) : null;
              const nextEntry  = entries ? nextInstallment(loan.id) : null;

              return (
                <div
                  key={loan.id}
                  className={`border rounded-xl transition-colors ${
                    isSuccess ? 'border-green-300 bg-green-50' : 'border-gray-100 bg-white hover:border-gray-200'
                  }`}
                >
                  {/* Ligne principale */}
                  <div className="flex items-center gap-4 p-4">
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-5 gap-3 items-center text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-blue-700">{fmt(loan.monthlyPayment)} FCFA</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400">Restant dû</p>
                        <p className="font-semibold text-gray-700">{fmt(loan.remainingBalance)} FCFA</p>
                      </div>
                      <div>
                  ...
                  {pct !== null && (
                    <div className="px-4 pb-2">
                      <div className="flex justify-between text-xs text-gray-400 mb-1">
                        <span>{paid}/{total} échéances payées</span>
                        <span>{pct}%</span>
                      </div>
                  ...
                  {/* Tableau d'amortissement déroulant */}
                  {expanded && (
                    <div className="border-t border-gray-100 px-4 pb-4 pt-3">
                      {!entries ? (
                        <p className="text-center text-gray-400 text-sm py-4">Chargement…</p>
                      ) : entries.length === 0 ? (
                        <p className="text-center text-gray-400 text-sm py-4">Plan non disponible</p>
                      ) : (
                            ...
                            <tbody>
                              {entries.map((e, i) => (
                                <tr key={i} className={`border-b border-gray-50 ${e.paid ? 'opacity-50' : ''}`}>
                                  <td className="py-1.5 font-medium text-gray-700">{e.installmentNumber ?? i + 1}</td>
      ...
      {modal && (
        <Modal
          isOpen
          onClose={() => { setModal(null); setErreur(''); }}
          ...
          <div className="space-y-4 min-w-[480px]">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="bg-amber-50 rounded-xl p-3 text-center">
                <p className="text-xs text-amber-500 mb-0.5">Prochain paiement</p>
                <p className="text-sm font-bold text-amber-800">
                  {modal.nextPaymentDate
                    ? new Date(modal.nextPaymentDate).toLocaleDateString('fr-FR')
                    : '—'}
                </p>
              </div>
            </div>
              ...
              return (
                <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 text-sm">
                  <p className="text-indigo-700 font-medium mb-1">
                    Prochaine échéance — N° {next.installmentNumber}
                  </p>
                  <div className="flex gap-6 text-xs text-indigo-600">
                    <span>Montant dû : <strong>{fmt(next.dueAmount)} FCFA</strong></span>
            ...
            <form onSubmit={handleEnregistrer} className="space-y-3">
              ...
              <div>
                <label className="form-label">Observations <span className="text-gray-400 font-normal">(optionnel)</span></label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={e => setNotes(e.target.value)}
                  placeholder="Paiement reçu en agence…"
              ...
              <div className="flex justify-end gap-3 pt-1">
                <button type="button" onClick={() => { setModal(null); setErreur(''); }} className="btn-secondary">
                  Annuler
                </button>
                <button
                  type="submit"
...
export default AgentRemboursementsPage;
