...
const OperationsPage = () => {
  ...
  const [compteSel,      setCompteSel]      = useState(null);   // objet compte complet
  ...
  useEffect(() => {
    getMyClients()
      .then(res => setMesClients(res.data?.content ?? res.data ?? []))
      .catch(() => setMesClients([]));
  }, []);
  ...
  const handleSelectClient = async (client) => {
    ...
    try {
      const res = await getComptesActifsByClient(client.id, 0, 50);
      setComptesClient(res.data?.data?.content ?? res.data?.data ?? []);
    } catch {
      setComptesClient([]);
    } finally {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm">Effectuez des opérations pour vos clients</p>
      </div>
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {clientsFiltres.length === 0 ? (
              <p className="text-center text-gray-300 text-sm py-8">
                {mesClients.length === 0 ? 'Aucun client assigné' : 'Aucun résultat'}
              </p>
            ) : (
            ...
            <>
              ...
              <div>
                ...
                {!compteSel ? (
                  ...
                  // Compte sélectionné — affichage compact
